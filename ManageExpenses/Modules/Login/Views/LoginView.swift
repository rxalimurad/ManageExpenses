            ...
            GeometryReader { _ in
                ZStack {
                    VStack {
                        InputWidgetView(
                            hint: "Email",
                            properties: InputProperties(maxLength: 40, regex: Constants.regex.email),
                        ...
                        InputWidgetView(
                            hint: "Password",
                            properties: InputProperties(maxLength: 20, minLength: 8, isSecure: true),
                            text: $viewModel.userDetails.password,
                            isValidField: $viewModel.isValidPassword
                        )
                        .padding([.top], 24)
                        .padding([.leading, .trailing], 16)
                        
                        ButtonWidgetView(title: "Login", style: .primaryButton) {
                            viewModel.signin()
                        }
                        .padding([.top], 40)
                ...
                .fullScreenCover(isPresented: $viewModel.showIntro) {
